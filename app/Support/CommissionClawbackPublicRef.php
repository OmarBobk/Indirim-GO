...
/**
 * Generates immutable clawback obligation references (CLB-XXXXXXXXXX).
 */
final class CommissionClawbackPublicRef
{
    public const PREFIX = 'CLB-';
    ...
    public static function allocateUnique(int $attempts = 5): string
    {
        ...
        for ($i = 0; $i < $attempts; $i++) {
            ...
            if (! CommissionClawback::query()->where('public_ref', $candidate)->exists()) {
                return $candidate;
            }
        }

        throw new \RuntimeException('Unable to allocate a unique commission clawback public reference.');
    }

    public static function isValidFormat(string $value): bool
    {
        return (bool) preg_match('/^CLB-[A-F0-9]{10}$/', strtoupper(trim($value)));
    }
    ...
    public static function withUniqueRetry(callable $persist, int $attempts = 5): mixed
    {
        ...
        throw $lastException ?? new \RuntimeException('Unable to persist commission clawback public reference.');
    }
