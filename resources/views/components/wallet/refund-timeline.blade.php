...
@unless ($timeline === [])
    <section class="storefront-card storefront-card--pad-md" data-test="refund-timeline" aria-labelledby="refund-timeline-heading">
        <flux:heading size="sm" id="refund-timeline-heading">{{ __('messages.refund_timeline_heading') }}</flux:heading>
        <ol class="mt-4 space-y-3">
            @foreach ($timeline as $event)
                <li class="flex gap-3" wire:key="refund-timeline-{{ $event['key'] ?? $loop->index }}">
