...
- **Mobile catalog (M2.1–M2.3, closed):** authenticated read endpoints `GET /api/v1/catalog/home`, `GET /api/v1/packages`, `GET /api/v1/packages/{id}` under `mobile-catalog` throttle. Final customer prices are server-calculated USD decimal strings plus trusted `FrontendMoney` display; Flutter must never compute authoritative prices. No shared public cache for priced responses. Flutter M2.2 discovery UI is on mobile `main`; local emulator base URL remains `http://10.0.2.2:8000/api/v1`. No purchasing in M2.
